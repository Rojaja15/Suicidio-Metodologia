...
# Asegurar eliminación por si cluster estaba en num
vars_num <- vars_num[vars_num != "cluster"]
...
walk(vars_num, function(v){
  p <- ggplot(df_final, aes(x = cluster, y = .data[[v]], fill = cluster)) +
    geom_violin(trim = FALSE, alpha = 0.6, color = "gray30") +
    geom_boxplot(width = 0.15, outlier.shape = NA, alpha = 0.3, color = "gray10") +
    stat_summary(fun = median, geom = "point", size = 2, color = "black") +
    scale_fill_manual(values = c("#00AFBB", "#E7B800", "#FC4E07")) + # paleta científica azul-gris
    labs(title = v, x = "Cluster", y = "Valor") +
    theme_minimal(base_size = 14) +
    theme(
