...
```{r}
var_results <- get_pca_var(resultado_acp)
```

```{r}
cos2_dims_1_2 <- var_results$cos2[, 1] + var_results$cos2[, 2]
umbral <- 0.4
```

```{r}
variables_fuertes <- names(cos2_dims_1_2[cos2_dims_1_2 > umbral])
```

```{r}
print(paste("Variables seleccionadas:", length(variables_fuertes)))
print(variables_fuertes)
```

```{r}
df_filtrado <- df_seleccionado[, variables_fuertes]

# 7. Volver a correr el ACP (prcomp)
acp_filtrado <- prcomp(df_filtrado, scale. = FALSE)
```

```{r}
summary(acp_filtrado)
```
```{r}
fviz_eig(acp_filtrado, addlabels = TRUE)
```

```{r}
fviz_pca_var(acp_filtrado,
             col.var = "contrib",
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
             repel = TRUE)
```
...
# Vector con las variables categóricas que NO queremos incluir en el ACM
variables_a_quitar <- c(
  "escolaridad", "orientacion_sexual", "provincia", "grupo", "canton", 
  "grupo_wem", "fecha", "dia", "EIS_Cat", "ocupacion"
)

# Filtramos el vector 'vars_cat' (que definiste en tu sección de bivariados)
# Nos quedamos solo con las variables que NO están en la lista 'variables_a_quitar'
vars_cat_filtradas <- vars_cat[!(vars_cat %in% variables_a_quitar)]
```

```{r}
# 2. Crear el dataframe para el ACM usando la lista filtrada
df_categorico <- df %>% 
  select(all_of(vars_cat_filtradas)) %>%  # <-- Usamos la lista filtrada aquí
  mutate(across(everything(), as.factor)) %>%
  na.omit() # Asegúrate de manejar los NAs

# 3. Correr el modelo ACM
# (El resto de tu código de ACM sigue exactamente igual...)
modelo_acm <- MCA(df_categorico, ncp = 5, graph = FALSE)

# 4. Ver la inercia explicada
fviz_eig(modelo_acm, addlabels = TRUE, title = "ACM - Gráfico de Sedimentación")
```

```{r}
fviz_contrib(modelo_acm, 
             choice = "var", 
             axes = c(1, 2),
             top = 20,
             ggtheme = theme_minimal()) +
  labs(title = "Contribución de Variables Categóricas a Dim 1 y 2") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```

```{r}
vars_categoricas_fuertes <- c(
  "ruptura_reciente_Si",
  "medidas_cautelares_Si",
  "crisis_pareja_actual_Si",
  "crisis_pareja_actual_No",
  "casado",                   # <-- Corregido
  "medicamento_psiquiatrico_Si",
  "ruptura_reciente_No",
  "estado_civil_Viudo",       # <-- Corregido
  "separado"                  # <-- Corregido
)
```

```{r}
# 2. Variables numéricas (del PCA)
#    (Este vector ya lo tienes de tu paso de PCA)
vars_numericas_fuertes <- names(df_filtrado)

# 3. Combinar ambos vectores
variables_finales <- c(vars_numericas_fuertes, vars_categoricas_fuertes)

# 4. Crear el df_filtrado_final
#    Este comando ahora SÍ funcionará
df_filtrado_final <- df_escalado %>%
